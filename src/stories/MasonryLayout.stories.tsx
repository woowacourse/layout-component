...
const getRandomImageUrl = () => {
  const randomIndex = Number((Math.random() * IMAGE_URL.length - 1).toFixed(0));

  return IMAGE_URL[randomIndex];
};

const MOCK_RANDOM_IMAGE_LIST = new Array(5).fill(1).map(() => {
  return getRandomImageUrl();
});

function MasonryLayoutStory(props: ComponentProps) {
  console.log(MOCK_RANDOM_IMAGE_LIST);
  return (
    <MasonryLayout {...props}>
      {MOCK_RANDOM_IMAGE_LIST.map(item => (
        <Image key={item} src={item} alt="랜덤 이미지" />
      ))}
    </MasonryLayout>
  );
}
